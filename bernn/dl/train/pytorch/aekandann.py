...
class SHAPKANAutoencoder3(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_emb, n_meta, mapper, variational, layer1, layer2, dropout,
        ...
        self.flow_type = 'vanilla'
        # self.gnn1 = GCNConv(in_shape, in_shape)
        self.enc = KAN([in_shape + n_meta, layer1, layer2], device=device)
        if conditional:
            self.dec = KAN([in_shape + n_meta, n_batches, layer2, layer1], device=device)
        else:
            self.dec = KAN([in_shape + n_meta, 0, layer2, layer1], device=device)
        self.mapper = KAN([n_batches + 1, layer2], device=device)

        if variational:
            self.gaussian_sampling = GaussianSample(layer2, layer2, device)
        else:
            self.gaussian_sampling = None
        self.dann_discriminator = KAN([layer2, 64, n_batches])
        self.classifier = KAN([layer2 + n_emb, nb_classes])
        self.random_init(nn.init.xavier_uniform_)
...
class KANAutoencoder3(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb, mapper, variational, layer1, layer2, dropout, n_layers, zinb=False,
        ...
        if variational:
            # TODO GaussianSample should also be using KANs
            self.gaussian_sampling = GaussianSample(layer2, layer2, device)
        else:
            self.gaussian_sampling = None
        self.dann_discriminator = KAN([layer2, 64, n_batches], device=device)
        self.classifier = KAN([layer2 + n_emb, nb_classes], device=device)
        self.random_init(nn.init.kaiming_uniform_)
