...
class AutoEncoder3(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb,
                 mapper, variational, layers, dropout, n_layers,
                 prune_threshold, zinb=False, conditional=False,
                 add_noise=False, tied_weights=0, update_grid=False,
                 use_gnn=False, device='cuda'):
        """
        ...
        self.flow_type = 'vanilla'
        # self.gnn1 = GCNConv(in_shape, in_shape)
        self.enc = Encoder3(in_shape + n_meta, layers, dropout)
        if conditional:
            self.dec = Decoder3(in_shape + n_meta, n_batches, layers, dropout)
        else:
            self.dec = Decoder3(in_shape + n_meta, 0, layers, dropout)
        self.mapper = Classifier(n_batches + 1, layers[-1])

        if variational:
            self.gaussian_sampling = GaussianSample(layers[-1], layers[-1], device)
        else:
            self.gaussian_sampling = None
        self.dann_discriminator = Classifier2(layers[-1], 64, n_batches)
        self.classifier = Classifier(layers[-2] + n_emb, nb_classes, n_layers=n_layers)
        self._dec_mean = nn.Sequential(nn.Linear(layers[-2], in_shape + n_meta), MeanAct())
        self._dec_disp = nn.Sequential(nn.Linear(layers[-2], in_shape + n_meta), DispAct())
        self._dec_pi = nn.Sequential(nn.Linear(layers[-2], in_shape + n_meta), nn.Sigmoid())
        self.random_init(nn.init.kaiming_uniform_)
