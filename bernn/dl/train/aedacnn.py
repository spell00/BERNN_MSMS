...
class Encoder32(nn.Module):
    def __init__(self, ni, dropout):
        super(Encoder32, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(ni, 32, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(32),
            nn.Dropout(dropout),
            nn.ReLU(),
        )
    ...
    def forward(self, ins):
        ...
        x, inds = self.maxpool(x)
        indices += [inds]
        return x.squeeze(-1).squeeze(-1), indices
...
class Encoder64(nn.Module):
    def __init__(self, ni, dropout):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(ni, 32, kernel_size=(3, 3), stride=1, padding=1),
            nn.BatchNorm2d(32),
            nn.Dropout(dropout),
            nn.ReLU(),
        )
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=(3, 3), padding=0),
            nn.BatchNorm2d(512),
            nn.Dropout(dropout),
            nn.ReLU(),
        )
...
class Decoder32(nn.Module):
    def __init__(self, ni, dropout):
        ...
        self.deconv4 = nn.Sequential(
            nn.ConvTranspose2d(32, ni, kernel_size=(3, 3), stride=(1, 1), padding=1),
        )
...
class Decoder64(nn.Module):
    def __init__(self, ni, dropout):
        ...
        self.deconv2 = nn.Sequential(
            nn.ConvTranspose2d(256, 128, kernel_size=(3, 3), stride=(1, 1), padding=0),
            nn.BatchNorm2d(128),
            nn.Dropout(dropout),
            nn.ReLU(),
        )
        self.deconv1 = nn.Sequential(
            nn.ConvTranspose2d(512, 256, kernel_size=(3, 3), stride=(1, 1), padding=0),
            nn.BatchNorm2d(256),
            nn.Dropout(dropout),
            nn.ReLU(),
        )
    ...
    def forward(self, inp, indices):
        # x = self.maxunpool(inp.unsqueeze(2).unsqueeze(2), indices[3])
        try:
            x = self.deconv1(inp.unsqueeze(2).unsqueeze(2))
        except:
            pass
        x = self.maxunpool(x, indices[3])
        x = self.deconv2(x)
        x = self.maxunpool(x, indices[2])
        x = self.deconv3(x)
        x = self.maxunpool(x, indices[1])
...
class SHAPAutoEncoder32(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_emb, n_meta, mapper, variational, dropout,
        ...
        super(SHAPAutoEncoder32, self).__init__()
        self.n_emb = n_emb
        ...
        self.enc = Encoder32(in_shape, dropout)
        if conditional:
            self.dec = Decoder32(in_shape + n_meta, dropout)
        else:
            self.dec = Decoder32(in_shape + n_meta, dropout)
...
class SHAPAutoEncoder64(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_emb, n_meta, mapper, variational, dropout,
                 n_layers, zinb=False, conditional=False, add_noise=False, tied_weights=0, use_gnn=False, device='cuda'):
        super(SHAPAutoEncoder64, self).__init__()
        self.n_emb = n_emb
        self.add_noise = add_noise
        self.n_meta = n_meta
        self.device = device
        ...
        self.flow_type = 'vanilla'
        # self.gnn1 = GCNConv(in_shape, in_shape)
        self.enc = Encoder64(in_shape, dropout)
        if conditional:
            self.dec = Decoder64(in_shape + n_meta, dropout)
        else:
            self.dec = Decoder64(in_shape + n_meta, dropout)
        self.mapper = Classifier(n_batches + 1, 512, n_layers=2)

        if variational:
            self.gaussian_sampling = GaussianSample(512, 512, device)
        else:
            self.gaussian_sampling = None
        self.dann_discriminator = Classifier2(512, 512, n_batches)
        self.classifier = Classifier(256 + n_emb, nb_classes, n_layers=n_layers)
        self.random_init(nn.init.xavier_uniform_)
...
class AutoEncoder32(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb, mapper, variational, dropout, n_layers, zinb=False,
        ...
        self.flow_type = 'vanilla'
        # self.gnn1 = GCNConv(in_shape, in_shape)
        self.enc = Encoder32(in_shape + n_meta, dropout)
        if conditional:
            self.dec = Decoder32(in_shape + n_meta, dropout)
        else:
            self.dec = Decoder32(in_shape + n_meta, dropout)
...
class AutoEncoder64(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb, mapper, variational, dropout, n_layers, zinb=False,
        ...
        self.flow_type = 'vanilla'
        # self.gnn1 = GCNConv(in_shape, in_shape)
        self.enc = Encoder64(in_shape + n_meta, dropout)
        if conditional:
            self.dec = Decoder64(in_shape + n_meta, dropout)
        else:
            self.dec = Decoder64(in_shape + n_meta, dropout)
        ...
        if variational:
            ...
            self.gaussian_sampling = None
        self.dann_discriminator = Classifier2(512, 512, n_batches)
        self.classifier = Classifier(512 + n_emb, nb_classes, n_layers=n_layers)
        # self._dec_mean = nn.Sequential(nn.Linear(layer1, in_shape + n_meta), MeanAct())
        # self._dec_disp = nn.Sequential(nn.Linear(layer1, in_shape + n_meta), DispAct())
        # self._dec_pi = nn.Sequential(nn.Linear(layer1, in_shape + n_meta), nn.Sigmoid())
        self.random_init(nn.init.kaiming_uniform_)
...
class SHAPAutoEncoder3(nn.Module):
    ...
    def forward(self, x, batches=None, sampling=False, beta=1.0):
        ...
        #     x = self.gnn1(x)
        try:
            enc, indices = self.enc(x).squeeze()
        except:
            pass
        if self.gaussian_sampling is not None:
            if sampling:
