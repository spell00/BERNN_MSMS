...
class SHAPKANAutoEncoder3(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb, mapper, variational, layers: dict,
        ...
        self.is_sigmoid = is_sigmoid  # Store the parameter
        ...
        if zinb:
            self._dec_mean = nn.Sequential(
                KANLayer(layers[list(layers.keys())[-2]], in_shape + n_meta, device=device), MeanAct())
            self._dec_disp = nn.Sequential(
                KANLayer(layers[list(layers.keys())[-2]], in_shape + n_meta, device=device), DispAct())
            self._dec_pi = nn.Sequential(
                KANLayer(layers[list(layers.keys())[-2]], in_shape + n_meta, device=device), nn.Sigmoid())
    ...
    def random_init(self, init_func=nn.init.kaiming_uniform_):
        for m in self.modules():
            if isinstance(m, KANLayer) or isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                init_func(m.weight.data)
                if m.bias is not None:
                    m.bias.data.zero_()
