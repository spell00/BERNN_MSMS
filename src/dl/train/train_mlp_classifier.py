...
class MLP(nn.Module):
    ...
    def __init__(self, n_in, n_classes, n_batches=2):
        super(MLP, self).__init__()

        self.network = nn.Sequential()
        self.network.add_module('drop0', nn.Dropout())
        self.network.add_module('c_fc1', nn.Linear(n_in, 32))
        self.network.add_module('bn1', nn.BatchNorm1d(32))
        self.network.add_module('drop1', nn.Dropout())

        self.classifier = nn.Sequential()
        self.classifier.add_module('c_fc1', nn.Linear(32, n_classes))

        self.dann_discriminator = nn.Sequential()
        self.dann_discriminator.add_module('d_fc1', nn.Linear(32, n_batches))
...
class TrainAE:
    ...
    def train(self, params):
        ...
        if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
            ...
            cnn = MLP(data['inputs']['train'].shape[-1] * data['inputs']['train'].shape[-2], self.n_cats, self.n_batches).to(self.args.device)
            ...
            for epoch in range(0, self.args.n_epochs):
                if early_stop_counter >= self.args.early_stop:
                    if self.verbose > 0:
                        ...
                        print(f"Final Classification Mcc Epoch {epoch}, "
    ...
    def loop(self, group, optimizer_ae, ae, celoss, triplet_loss, loader, lists, traces, nu=1, mapping=True):
        ...
        with tqdm(total=len(loader), position=0, leave=True) as pbar:
            for i, batch in enumerate(loader):
                ...
                # else:
                enc = ae(data.view(data.shape[0], -1))
...
if __name__ == "__main__":
    ...
    parameters = [
        ...
        {"name": "lr", "type": "range", "bounds": [1e-3, 1e-2], "log_scale": True},  # Best run: [1e-4, 1e-1]
