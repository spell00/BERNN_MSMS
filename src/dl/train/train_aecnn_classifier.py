...
class TrainAE:
    ...
    def train(self, params):
        ...
        if self.args.dataset == 'bacteria':
            ...
            self.data, self.unique_labels, self.unique_batches = get_mnist(self.path, args, seed=seed)
            self.pools = False
        else:
        ...
        with tqdm(total=len(loader), position=0, leave=True) as pbar:
            for i, all_batch in iterator:
                ...
                if args.dloss not in ['revTriplet', 'inverseTriplet']:
                    ...
                    pos_batch_sample, neg_batch_sample = neg_batch_sample.to(self.args.device).float(), pos_batch_sample.to(self.args.device).float()
                    meta_pos_batch_sample, meta_neg_batch_sample = meta_neg_batch_sample.to(self.args.device).float(), meta_pos_batch_sample.to(self.args.device).float()
                    if self.args.n_meta > 0:
                        pos_batch_sample = torch.cat((pos_batch_sample, meta_pos_batch_sample), 1)
                        neg_batch_sample = torch.cat((neg_batch_sample, meta_neg_batch_sample), 1)
                    pos_enc, _, _, _ = ae(pos_batch_sample.unsqueeze(1), pos_batch_sample, domain, sampling=True)
