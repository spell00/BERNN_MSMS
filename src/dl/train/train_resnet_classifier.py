...
from src.utils.data_getters import get_bacteria_resnet, get_cifar10, get_mnist
...
from src.dl.models.pytorch.utils.dataset import get_loaders, get_images_loaders, get_images_loaders_no_pool
from src.utils.utils import scale_data, get_unique_labels, to_csv
...
class TrainAE:
    ...
    def train(self, params):
        ...
        if self.args.dataset == 'bacteria':
            self.data, self.unique_labels, self.unique_batches = get_bacteria_resnet(self.path, self.args, seed=42)
            self.pools = False
        ...
        if self.log_neptune:
            ...
            run = neptune.init_run(
                project="simonjpelletier2/BatchEffect",
        ...
        if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
            ...
            cnn = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=True).to(self.args.device)
            ...
            cnn.fc = Identity()
            cnn.classifier = nn.Linear(in_features=512, out_features=self.n_cats).to(self.args.device)
            cnn.dann_discriminator = nn.Linear(in_features=512, out_features=self.n_batches).to(self.args.device)
            ...
            for epoch in range(0, self.args.n_epochs):
                ...
                with torch.no_grad():
                    for group in list(data['inputs'].keys()):
                        if group in ['train', 'all', 'all_pool']:
                            continue
                        closs, lists, traces = self.loop(group, optimizer_ae, cnn, sceloss, triplet_loss,
                ...
                if np.mean(values['valid']['mcc'][-self.args.n_agg:]) > self.best_mcc and len(
                    ...
                    torch.save(cnn.state_dict(), f'{self.complete_log_path}/model.pth')
            ...
            cnn.load_state_dict(torch.load(f'{self.complete_log_path}/model.pth'))
            # Need another model because the other cant be use to get shap values
            # shap_cnn.load_state_dict(torch.load(f'{self.complete_log_path}/model.pth'))
    ...
    def loop(self, group, optimizer_ae, ae, celoss, triplet_loss, loader, lists, traces, nu=1, mapping=True):
        """
        ...
        with tqdm(total=len(loader), position=0, leave=True) as pbar:
            for i, batch in enumerate(loader):
                ...
                # else:
                enc = ae(data.unsqueeze(1).repeat(1, 3, 1, 1)).squeeze(-1).squeeze(-1)
                # enc = ae.avgpool(enc)
                ...
                if args.dloss not in ['revTriplet', 'inverseTriplet']:
                    ...
                    pos_enc = ae(pos_batch_sample.unsqueeze(1).repeat(1, 3, 1, 1)).squeeze(-1).squeeze(-1)
                    neg_enc = ae(neg_batch_sample.unsqueeze(1).repeat(1, 3, 1, 1)).squeeze(-1).squeeze(-1)
                ...
                try:
                    ...
                    pass
                if group == 'train':
                    preds = preds[[s == 'train' for s in set]]
                    labels = labels[[s == 'train' for s in set]]
    ...
    def forward_discriminate(self, optimizer_b, ae, celoss, loader):
        ...
        for i, batch in enumerate(loader):
            ...
            to_rec = to_rec.to(self.args.device).float()
            with torch.no_grad():
                enc, rec, _, kld = ae(data.unsqueeze(1), to_rec, domain, sampling=sampling)
            with torch.enable_grad():
                domain_preds = ae.dann_discriminator(enc)
    ...
    def freeze_dlayers(self, ae):
        """
        Freeze all layers except the dann discriminator
        Args:
...
if __name__ == "__main__":
    ...
    if args.dloss in ['revTriplet', 'revDANN', 'DANN', 'inverseTriplet', 'normae']:
        # gamma = 0 will ensure DANN is not learned
        parameters += [
            {"name": "gamma", "type": "range", "bounds": [1e-2, 1e2], "log_scale": True}]  # Best run: [1e-8, 1e2]
    if args.variational:
        # beta = 0 because useless outside a variational autoencoder
